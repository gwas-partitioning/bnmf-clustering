...
version = "bNMF_example"
main_dir <- file.path(getwd(), paste0(version,"_results"))
dir.create(main_dir)
...
rsID_map_file <- "/humgen/florezlab/users/clairekim/list_VARID_rsID_updated.txt"  # From dbSNP v1.38 -- maps positional IDs to rsIDs

# GWAS for main trait
gwas <- read_excel("./clustering_data_sources_example.xlsx",
                   sheet="main_gwas") %>%
  data.frame()

# GWAS for clustering traits
gwas_traits <- read_excel("./clustering_data_sources_example.xlsx",
                          sheet="trait_gwas")
...
# 
# save.image(file = sprintf("%s.RData", version))
# 
# # #----
# # 
# # # SECTION 3A: VARIANT CLUMPING (POSITION-BASED)
# # 
...
# ld_files <- list.files(path=main_dir,
...
# api_path <- "/humgen/florezlab/users/ksmith/topld_api/topld_api"
...
#             file = file.path(main_dir, "rsID_map.txt"),
#             col_names = T)
# 
# save.image(file = sprintf("%s.RData", version))
# 
...
bnmf_reps <- run_bNMF(final_zscore_matrix,
                      n_reps=10,
                      tolerance = 1e-5)
summarize_bNMF(bnmf_reps, dir_save=main_dir)
...
file.rename(from = file.path("/humgen/florezlab/users/ksmith/",html_filename),
            to = file.path(main_dir, html_filename))
